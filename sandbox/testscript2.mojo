...
fn main() raises:
    ...
    var np_array = np.array([1, 2, 3, 4, 5, 6], dtype=np.int32)
    np_array = np_array.reshape(2, 3)

    # Create 1D NumPy arrays
    var array1 = np.array([1, 2], dtype = np.int32)
    var array2 = np.array([3, 4], dtype = np.int32)
    var array3 = np.array([5, 6], dtype = np.int32)
    var result = array1 + array2 + array3
    
    # Test 2D > array(s) with Bridge module
    var array4 = np.array([1, 2, 3, 4, 5], dtype = np.int32)
